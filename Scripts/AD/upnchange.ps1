...
foreach ($textUser in $txtUsers) {
    ...
    if ($userExists -eq $null) {
        ...
        $noneDomainEmail = $findEmail -replace "@domain.com", "" | Out-String
        Write-Log "Removed KCS domain from string output is $noneDomainEmail"
        $newUPN = noneDomainEmail + "@" + $newSuffix
        Set-ADUser -UserPrincipalName $newUPN -Identity $textUser
